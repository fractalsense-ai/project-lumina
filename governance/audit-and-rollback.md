...
This document specifies audit procedures and rollback policy for Project Lumina. It governs how CTL records are reviewed, how discrepancies are handled, and what rollback means in an append-only system.
...
The CTL hash chain must be verified periodically to detect tampering.

### Automated Scan

An automated scan should run monthly (minimum) against the full CTL:
...
2. **Record**: Append a `TraceEvent` with `event_type: chain_integrity_failure` to the CTL
3. **Escalate**: Create an `EscalationRecord` to the highest available Meta Authority
4. **Preserve**: Do not modify the CTL — preserve it exactly as found
...
The CTL is **append-only**. Rollback does not mean deleting records. It means:
...
4. Sessions started before this CommitmentRecord retain their original domain pack hash in their CTL records
...
Profile rollback requires Meta Authority approval and is recorded in the CTL.

### What Cannot Be Rolled Back

- CTL records themselves cannot be deleted or modified
...
A security event is any situation where the CTL integrity is intentionally compromised, PII is exposed, or the system is operated outside its consent boundaries.

Security event response:

1. **Immediate freeze**: All sessions institution-wide
2. **Preserve**: CTL is preserved exactly as found (no modifications)
