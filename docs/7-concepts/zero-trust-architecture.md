...
| **Caller identity** | Unauthorized user accesses CTL records, domain physics, or executes a session | JWT authentication on all API endpoints; RBAC permission check on every request |
...
| **Ledger integrity** | An audit record is modified after the fact to conceal an escalation | Append-only CTL with SHA-256 hash-chain; `validate_ctl_chain` endpoint detects any tampering |
| **System state** | System-level rules are changed without an auditable record | System physics hash injected into every CTL TraceEvent metadata; system has its own append-only CTL |
...
| **Policy commitment gate** | No domain configuration is trusted at rest without a hash commitment | At session start: SHA-256 of active domain-physics.json compared to CTL CommitmentRecord `subject_hash`; `LUMINA_ENFORCE_POLICY_COMMITMENT` controls enforcement | **Session rejected** — no session executes without a matching CommitmentRecord |
...
| **5. Monitor and measure integrity of all assets** | The enterprise monitors and measures the integrity and security posture of all owned assets | `validate_ctl_chain` checks SHA-256 hash-chain integrity across all CTL records; `lumina-integrity-check` verifies SHA-256 of all repo artifacts against `docs/MANIFEST.yaml`; system physics hash is injected per turn |
...
| **7. Collect information to improve security posture** | Collect as much information as possible about assets, infrastructure, and communications | CTL collects structured telemetry on every decision (not transcript content — see Principle 2: Measurement, Not Surveillance); escalation records are permanent; provenance metadata links every turn to the domain physics hash and system physics hash at that moment |
...
- Audit logs in the CTL contain pseudonymous tokens, not personal identifiers
...
The CTL cannot be modified. Any attempt to alter a historical record breaks the hash-chain and is detectable by `validate_ctl_chain`. This provides:
...
- A foundation for compliance audits — auditors read the CTL with confidence that its content has not been altered
...
1. Log the escalation event to CTL with a full provenance record
...
- CTL hash-chain break → reported as MISMATCH, not silently ignored
...
- [`standards/lumina-core-v1.md`](../../standards/lumina-core-v1.md) — system CTL, policy commitment gate, system physics hash, dual-ledger cross-reference chain
- [`standards/causal-trace-ledger-v1.md`](../../standards/causal-trace-ledger-v1.md) — CTL hash-chain specification, record types, append-only guarantees
